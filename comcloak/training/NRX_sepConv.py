...
from keras.layers import Layer, LayerNormalization, Conv2D, Conv2DTranspose, Dense, LSTM, Bidirectional, Embedding, SeparableConv2D
...
tdl_model = "C" # TDL model to use
validation_cdl_model = "C" # CDL model to use for validation
speed = 1.0 # Speed for evaluation and training [m/s]
# SNR range for evaluation and training [dB]
ebno_db_min = 5.0
ebno_db_max = 25.0
val_ebno_db_min = -5.0
val_ebno_db_max = 25.0
############################################
...
training_batch_size = 90 # Training batch size
...
model_weights_path = "./comcloak/training/weights/Receiver_sys_weights_v8" # Location to save the neural receiver weights once training is done
train_log_path = "./comcloak/training/train_log/nrx_log/RX_log_v10_sepConvResFiLM_H_input.txt"
############################################
...
class CondConv2D(Layer):
    ...
    def call(self, inputs, cond):
        """
        inputs: [batch, H, W, C_in] # (300, 14, 128, 128)
        cond:   [batch, cond_dim]#(300, 8)
        """
        batch_size = tf.shape(inputs)[0]

        # α coefficients (batch, K)
        alphas = self.alpha_layer(cond)# (300, 4)

        # Mixture of convolution kernels (batch, kh, kw, Cin, Cout)
        kernels = tf.einsum("bk,k...->b...", alphas, self.expert_kernels)# (300, 3, 3, 128, 6)
        bias = tf.einsum("bk,kf->bf", alphas, self.expert_bias)# (300, 6)
...
class ResidualBlock(Layer):
    ...
    def call(self, inputs):
        z = self._layer_norm_1(inputs)# inputs&z (300,14,128,128)
        z = nn.relu(z)
        z = self._conv_1(z)
...
class ResidualBlock_FiLM(Layer):
    r"""
    This Keras layer implements a convolutional residual block made of two convolutional layers with ReLU activation, layer normalization, and a skip connection.
    The number of convolutional channels of the input must match the number of kernel of the convolutional layers ``num_conv_channel`` for the skip connection to work.
    ...
    def build(self, input_shape):
        ...
        self._layer_norm_1 = LayerNormalization(axis=(-1, -2, -3))
        # self._conv_1 = CondConv2D(filters=num_conv_channels, kernel_size=3, num_experts=4, cond_dim=16)
        self._conv_1 = SeparableConv2D(filters=num_conv_channels,
                              kernel_size=[3,3],
                              padding='same',
                              activation=None)
        self._film = FiLM(
            feature_dim=num_conv_channels, 
        )
        # Layer normalization is done over the last three dimensions: time, frequency, conv 'channels'
        self._layer_norm_2 = LayerNormalization(axis=(-1, -2, -3))
        self._conv_2 = SeparableConv2D(filters=num_conv_channels,
                              kernel_size=[3,3],
                              padding='same',
                              activation=None)

    def call(self, inputs, cond_vec):
        z = self._layer_norm_1(inputs)# inputs&z (300,14,128,128)
        z = nn.relu(z)
        z = self._conv_1(z)
        z = self._film(z, cond_vec)
        z = self._layer_norm_2(z)
        z = nn.relu(z)
        z = self._conv_2(z) # [batch size, num time samples, num subcarriers, num_channels]
        # Skip connection
...
class Receiver_Network(Model):
    ...
    def build(self, input_shape):
        ...
        # Input convolution
        self._input_conv = SeparableConv2D(filters=num_conv_channels,
                                   kernel_size=[3,3],
                                   padding='same',
                                   activation=None)
        # Residual blocks
        self._res_block_1 = ResidualBlock_FiLM()
        self._res_block_2 = ResidualBlock_FiLM()
        # self._res_block_3 = ResidualBlock_FiLM()
        # self._res_block_4 = ResidualBlock_FiLM()
        # Output conv
        ...
        self._output_conv = SeparableConv2D(filters=max_num_bits_per_symbol,
                                   kernel_size=[3,3],
                                   padding='same',
                                   activation=None)
    ...
    def call(self, inputs):
        ...
        mcs_label = insert_dims(cond_vec, 2, 1)
        mcs_label = tf.tile(mcs_label, [1, y.shape[1], y.shape[2], 1])
        ...
        # Residual blocks
        z = self._res_block_1(z, cond_vec)
        z = self._res_block_2(z, cond_vec)
        # z = self._res_block_3(z, cond_vec)
        # z = self._res_block_4(z, cond_vec)
        # Output conv
...
class E2ESystem(Model):
    def __init__(self, system, training=False, channel_model="CDL", validate=False):
        super().__init__()
        ...
        self._validate_channel = CDL(validation_cdl_model, delay_spread, carrier_frequency,
                    ut_array, bs_array, "downlink", min_speed=speed)
    ...
    def call(self, ebno_db):
        ...
        if self.validate == True:
            val_h_freq = self._channel(self._validate_channel)
            x_rg, g = self._zf_precoder([x_rg, val_h_freq])
            y = self._channel_freq([x_rg, val_h_freq, no_])
...
for i in range(num_training_iterations):
    ...
    if i % 5 == 0:
        ...
        with open(train_log_path, "a") as f:
            ...
            f.write(f"{i},{tf.reduce_mean(ebno_db):.2f},{loss.numpy():.6f}\n")
            # f.write(f"{i},{loss.numpy():.4f},{ebno_db:.2f}\n")
...
# model.validate = True
# N_MC = 50        # Monte Carlo 次数（20~100 都可以）
# results = {
#     "ebno_db": [],
#     "ber1": [],
#     "ber2": [],
...
#         # stream 1
#         e1 = tf.reduce_sum(tf.cast(b_mcs[0] != b_hat[0], tf.int32))
#         n1 = tf.size(b_mcs[0])

#         # stream 2
#         e2 = tf.reduce_sum(tf.cast(b_mcs[1] != b_hat[1], tf.int32))
#         n2 = tf.size(b_mcs[1])

#         # stream 3
#         e3 = tf.reduce_sum(tf.cast(b_mcs[2] != b_hat[2], tf.int32))
#         n3 = tf.size(b_mcs[2])

#         err1 += e1.numpy(); tot1 += n1.numpy()
#         err2 += e2.numpy(); tot2 += n2.numpy()
#         err3 += e3.numpy(); tot3 += n3.numpy()
#         print(f"Iter {iter+1}/{N_MC} at SNR={ebno_db.numpy():.1f} dB: "
#         f"BER1={err1/tot1:.3e}, "
#         f"BER2={err2/tot2:.3e}, "
#         f"BER3={err3/tot3:.3e}")

#     results["ebno_db"].append(float(ebno_db.numpy()))
#     results["ber1"].append(err1 / tot1)
#     results["ber2"].append(err2 / tot2)
#     results["ber3"].append(err3 / tot3)


    


# np.save("/home/wzs/Project/sionna-main/comcloak/training/results/data/CDL-C_early_stop_ber_sepConv_2Res.npy", results)
# data = np.load("/home/wzs/Project/sionna-main/comcloak/training/results/data/CDL-C_early_stop_ber_sepConv_2Res.npy", allow_pickle=True).item()

# plt.figure(figsize=(7,5))
# plt.semilogy(data["ebno_db"], data["ber1"], "-o", label="4QAM")
# plt.semilogy(data["ebno_db"], data["ber2"], "-s", label="16QAM")
# plt.semilogy(data["ebno_db"], data["ber3"], "-^", label="64QAM")
...
# plt.legend()
# plt.tight_layout()
# plt.savefig("/home/wzs/Project/sionna-main/comcloak/training/results/CDL-C_early_stop_ber_sepConv_2Res.png")
# plt.close()
