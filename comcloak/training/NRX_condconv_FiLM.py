...
validation_cdl_model = "E" # CDL model to use for validation
...
model_weights_path = "./comcloak/training/weights/RX_sys_weights_a_v0" # Location to save the neural receiver weights once training is done
train_log_path = "./comcloak/training/train_log/nrx_log/RX_log_a_condconv_FiLM_v0.txt"
############################################
...
class E2ESystem(Model):
    def __init__(self, system, training=False, channel_model="CDL", validate=False):
        ...
        self._validate_channel = CDL(validation_cdl_model, delay_spread, carrier_frequency,
                    ut_array, bs_array, "downlink", min_speed=speed)
        # self._validate_channel = TDL(validation_cdl_model, delay_spread, carrier_frequency, min_speed=speed,
        #     num_rx_ant=num_ut_ant, num_tx_ant=num_bs_ant)
...
model.validate = True
N_MC = 100        # Monte Carlo 次数（20~100 都可以）
results = {
    "ebno_db": [],
    "ber1": [],
    "ber2": [],
...
np.save("/home/wzs/Project/sionna-main/comcloak/training/results/data/CDL-E_baseline-ls-estimation.npy", results)
data = np.load("/home/wzs/Project/sionna-main/comcloak/training/results/data/CDL-E_baseline-ls-estimation.npy", allow_pickle=True).item()
...
plt.legend()
plt.tight_layout()
plt.savefig("/home/wzs/Project/sionna-main/comcloak/training/results/baseline/CDL-E_baseline-ls-estimation_mc.png")
plt.close()
